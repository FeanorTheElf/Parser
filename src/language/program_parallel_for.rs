...
{
    pub fn new(pos: TextPosition, indices: Vec<Expression>, alias: Option<Name>, write: bool) -> Self
    {
        ArrayEntryAccess {
            pos: pos, 
            indices: indices, 
    ...
    fn evaluate_constant(expression: &Expression) -> Result<Result<i32, TextPosition>, CompileError>
    {
        match expression {
            Expression::Call(call) => {
                if call.function == BuiltInIdentifier::FunctionAdd {
                    call.parameters.iter().map(|param| Self::evaluate_constant(param)).sum()
                } else if call.function == BuiltInIdentifier::FunctionMul {
                    call.parameters.iter().map(|param| Self::evaluate_constant(param)).product()
                } else if call.function == BuiltInIdentifier::FunctionUnaryNeg {
                    Self::evaluate_constant(&call.parameters[0]).map(|result_x| result_x.map(|x| -x))
                } else {
                    Err(CompileError::new(call.pos(),
    ...
    fn add_function_application_to_transformation_matrix(call: &FunctionCall, index_variable_map: &HashMap<&Name, usize>, result: &mut VecRefMut<i32>) -> Result<(), CompileError>
    {
        if call.function == BuiltInIdentifier::FunctionAdd {
            for param in &call.parameters {
                Self::add_expression_transform_to_transformation_matrix(param, index_variable_map, result)?;
            }
        } else if call.function == BuiltInIdentifier::FunctionMul {
    ...
    fn add_expression_transform_to_transformation_matrix(expression: &Expression, index_variable_map: &HashMap<&Name, usize>, result: &mut VecRefMut<i32>) -> Result<(), CompileError>
    {
        match expression {
            Expression::Call(call) => {
                Self::add_function_application_to_transformation_matrix(call, index_variable_map, result)?;
            },
    ...
    fn calculate_transformation_matrix(&self, index_variables: &Vec<&Name>) -> Result<Matrix<i32>, CompileError>
    {
    ...
    pub fn get_transformation_matrix<'a>(&'a self, index_variables: &Vec<&Name>) -> Result<Ref<'a, Matrix<i32>>, CompileError>
    {
        ...
        if self.matrix_cache.borrow().as_ref().unwrap().as_ref().is_err() {
            return Err(self.matrix_cache.borrow().as_ref().unwrap().as_ref().unwrap_err().clone());
        } else {
            return Ok(Ref::map(self.matrix_cache.borrow(), |cache_value| cache_value.as_ref().unwrap().as_ref().unwrap()));
        }
    }
...
fn test_get_transformation_matrix() {
    let array_entry_access = ArrayEntryAccess::new(BEGIN, vec![
    ...
    let expected: Matrix<i32> = Matrix::new(Box::new([-2,  1,  2, -1,
                                                       6, -1,  1,  0,
                                                       0,  9,  0, -9]), 3);
    assert_eq!(expected, *array_entry_access.get_transformation_matrix(&vec![&Name::l("a"), &Name::l("b"), &Name::l("c")]).unwrap());
}

#[test]
fn test_get_transformation_matrix_non_affine_transform() {
    let array_entry_access = ArrayEntryAccess::new(BEGIN, vec![
        Expression::parse(&mut fragment_lex("-1 + a * (2 + x)")).unwrap(),
    ], None, true);
    assert!(array_entry_access.get_transformation_matrix(&vec![&Name::l("a"), &Name::l("x")]).is_err());
}

#[test]
fn test_get_transformation_matrix_non_index_variable() {
    let array_entry_access = ArrayEntryAccess::new(BEGIN, vec![
        Expression::parse(&mut fragment_lex("a")).unwrap(),
    ], None, true);
    assert!(array_entry_access.get_transformation_matrix(&vec![&Name::l("i")]).is_err());
}