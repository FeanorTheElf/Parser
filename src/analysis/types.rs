...
enum ConcreteViewComputationResult {
    NotNecessary, UsedConcreteViewsNotKnown, Success
}
...
fn set_builtin_call_result_concrete_view(call: &FunctionCall, op: BuiltInIdentifier, scopes: &DefinitionScopeStack, type_lifetime: &mut LifetimeMut) -> ConcreteViewComputationResult
{
    ...
    match op {
        BuiltInIdentifier::ViewZeros => {
            ...
            ConcreteViewComputationResult::Success
        },
        BuiltInIdentifier::FunctionIndex => {
            ...
            let concrete: Box<dyn ConcreteView> = match indexed_type.deref(type_lifetime.as_const()) {
                Type::View(view) => {
                    ...
                    if let Some(indexed_concrete_view) = &view.concrete {
                        ...
                        return ConcreteViewComputationResult::UsedConcreteViewsNotKnown;
                    }
                },
                Type::Array(arr) => {
        ...
        {
            ConcreteViewComputationResult::NotNecessary
        }
    }
}
...
fn set_defined_call_result_concrete_view(call: &FunctionCall, scopes: &DefinitionScopeStack, types: &mut TypeVec) -> ConcreteViewComputationResult
{
    ...
    if let VoidableTypePtr::Some(ptr) = result_ptr {
        if ptr.deref(types.get_lifetime()).is_code_determined() {
            return ConcreteViewComputationResult::NotNecessary;
        }
        ...
        return ConcreteViewComputationResult::Success;
...
fn set_expression_concrete_view(expr: &Expression, scopes: &DefinitionScopeStack, types: &mut TypeVec) -> ConcreteViewComputationResult {
    match expr {
        Expression::Call(call) => {
            for param in &call.parameters {
                if set_expression_concrete_view(param, scopes, types) == ConcreteViewComputationResult::UsedConcreteViewsNotKnown {
                    return ConcreteViewComputationResult::UsedConcreteViewsNotKnown;
                }
            }
            match &call.function {
                Expression::Variable(var) => match &var.identifier {
                    Identifier::BuiltIn(op) => {
                        if set_builtin_call_result_concrete_view(call, *op, scopes, &mut types.get_lifetime_mut()) == ConcreteViewComputationResult::UsedConcreteViewsNotKnown {
                            return ConcreteViewComputationResult::UsedConcreteViewsNotKnown;
                        }
                    },
                    Identifier::Name(_) => {
                        if set_defined_call_result_concrete_view(call, scopes, types) == ConcreteViewComputationResult::UsedConcreteViewsNotKnown {
                            return ConcreteViewComputationResult::UsedConcreteViewsNotKnown;
                        }
                    }
                },
                _ => {
                    if set_expression_concrete_view(&call.function, scopes, types) == ConcreteViewComputationResult::UsedConcreteViewsNotKnown {
                        return ConcreteViewComputationResult::UsedConcreteViewsNotKnown;
                    }
                    if set_defined_call_result_concrete_view(call, scopes, types) == ConcreteViewComputationResult::UsedConcreteViewsNotKnown {
                        return ConcreteViewComputationResult::UsedConcreteViewsNotKnown;
    ...
    if !get_expression_type(expr, scopes).is_void() && get_expression_type(expr, scopes).unwrap().deref(types.get_lifetime()).is_view() {
        return ConcreteViewComputationResult::Success;
    } else {
        return ConcreteViewComputationResult::NotNecessary;
    }
