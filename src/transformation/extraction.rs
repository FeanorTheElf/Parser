...
use super::function_resolution::{find_function_definition, FunctionDefinition, DefinedFunctions};
    ...
    where F: FnMut(&FunctionCall, &Function) -> bool
{
        ...
        'b: 'a,
    {
        ...
        let recursive_extract = |expr: Expression| {
            self.extract_calls_in_expr(expr, rename_disjunct, defined_functions, previous_declaration_statements)
        };
        if let Some(return_type) = &function_definition.return_type {
            let variable_name = (*rename_disjunct)(Name::new(format!("result_{}", function_definition.identifier.name), 0));
            let declaration = LocalVariableDeclaration {
                ...
                value: Some(Expression::Call(Box::new(self.extract_calls_in_parameters(function_call, rename_disjunct, defined_functions, previous_declaration_statements)))),
            };
            ...
            return Expression::Call(Box::new(self.extract_calls_in_parameters(function_call, rename_disjunct, defined_functions, previous_declaration_statements)));
        }
    }
