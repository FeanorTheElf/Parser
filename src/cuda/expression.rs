...
pub fn gen_array_size<'a>(_pos: &TextPosition, name: &'a Name, ty: &Type) -> impl 'a + Iterator<Item = (CudaType, CudaIdentifier)> {
...
pub fn gen_simple_expr_array_size<'a>(expr: &'a Expression, ty: &Type) -> impl 'a + Iterator<Item = (CudaType, CudaExpression)> {
    match expr {
...
pub fn gen_variable(pos: &TextPosition, name: &Name, ty: &Type) -> (CudaType, CudaIdentifier) {
    match ty {
        Type::Array(base, _) => (gen_primitive_ptr_type(base, 1), CudaIdentifier::ValueVar(name.clone())),
...
pub fn gen_variables<'a>(pos: &'a TextPosition, var: &'a Name, ty: &'a Type) -> impl 'a + Iterator<Item = (CudaType, CudaIdentifier)> {
    ...
    std::iter::once(value).chain(size_data.into_iter().flat_map(move |_| gen_array_size(pos, var, ty)))
}
...
pub fn one_variable<I, R>(mut iter: I) -> (CudaType, R)
    where I: Iterator<Item = (CudaType, R)>
{
...
pub fn gen_variables_for_view<'a>(pos: &'a TextPosition, var: &'a Name, ty: &'a Type) -> impl 'a + Iterator<Item = (CudaType, CudaExpression)> {
    ...
    let size_data = if ty.is_array() { Some(()) } else { None };
    std::iter::once((cuda_type, expr)).chain(size_data.into_iter().flat_map(move |_| gen_array_size(pos, var, ty).map(|(cuda_type, cuda_var)| (cuda_type, CudaExpression::Identifier(cuda_var)))))
}
...
/// Array entries can be modified through the result of this function, but array sizes cannot. For real output parameters,
/// consider gen_variables_as_output_params() instead.
pub fn gen_variables_for_value<'a>(pos: &'a TextPosition, var: &'a Name, ty: &'a Type) -> impl 'a + Iterator<Item = (CudaType, CudaExpression)> {
    let (mut cuda_type, cuda_val) = gen_variable(pos, var, ty);
...
pub fn gen_output_parameter_declaration<'a>(_pos: &'a TextPosition, ty: &'a Type) -> impl 'a + Iterator<Item = (CudaType, CudaIdentifier)> {
    ...
    let params = call.parameters.iter().zip(param_types.iter()).flat_map(|(param, formal_param)| match param {
        Expression::Call(_) => {
            ...
            if formal_param.is_view() {
                Box::new(std::iter::once(gen_expression_for_view(param, context))) as Box<dyn Iterator<Item = Result<CudaExpression, OutputError>>>
            } else {
                Box::new(std::iter::once(gen_expression_for_value(param, context))) as Box<dyn Iterator<Item = Result<CudaExpression, OutputError>>>
            }
        },
        Expression::Literal(lit) => Box::new(std::iter::once(Ok(CudaExpression::IntLiteral(lit.value as i64)))) as Box<dyn Iterator<Item = Result<CudaExpression, OutputError>>>,
        Expression::Variable(var) => match &var.identifier {
            Identifier::Name(name) => if formal_param.is_view() {
                ...
                Box::new(gen_variables_for_value(var.pos(), name, &formal_param).map(|(_ty, p)| Ok(p))) as Box<dyn Iterator<Item = Result<CudaExpression, OutputError>>>
            },
    ...
    where I: Iterator<Item = &'a Expression>
{
    ...
    std::iter::once(index.map(|i| CudaExpression::AddressOf(Box::new(CudaExpression::Index(CudaIdentifier::ValueVar(array.clone()), Box::new(CudaExpression::Sum(i)))))))
}
...
{
    fn process_sum_prod<'stack, 'ast: 'stack, E>(call: &FunctionCall, toggle_unary_func: BuiltInIdentifier, default_e: &E, toggled_e: &E, context: &mut dyn CudaContext<'stack, 'ast>) -> Result<Vec<(E, CudaExpression)>, OutputError> 
    where E: Clone
    {
        call.parameters.iter().map(|p| {
            match p {
                Expression::Call(subcall) if subcall.function == toggle_unary_func => Ok((toggled_e.clone(), gen_expression_for_value(&subcall.parameters[0], context)?)),
    ...
    let mut cmp = |op: Cmp| -> Result<CudaExpression, OutputError> {
        Ok(CudaExpression::Comparison(op, Box::new(gen_expression_for_value(fst.unwrap(), context)?), Box::new(gen_expression_for_value(snd.unwrap(), context)?)))
    };
...
pub fn gen_expression_for_view<'stack, 'ast: 'stack>(expr: &Expression, context: &mut dyn CudaContext<'stack, 'ast>) -> Result<CudaExpression, OutputError>  {
    match expr {
        Expression::Call(call) => {
            if context.calculate_type(expr).is_view() {
                gen_function_call(call, std::iter::empty(), context)
            } else {
                Ok(CudaExpression::AddressOf(Box::new(gen_function_call(call, std::iter::empty(), context)?)))
            }
        },
        Expression::Variable(var) => match &var.identifier {
            Identifier::BuiltIn(_) => unimplemented!(),
...
pub fn gen_expression_for_value<'stack, 'ast: 'stack>(expr: &Expression, context: &mut dyn CudaContext<'stack, 'ast>) -> Result<CudaExpression, OutputError>  {
    match expr {
        Expression::Call(call) => {
            if context.calculate_type(expr).is_view() {
                Ok(CudaExpression::deref(gen_function_call(call, std::iter::empty(), context)?))
            } else {
                gen_function_call(call, std::iter::empty(), context)
            }
        },
...
pub fn gen_array_checked_copy_assignment_from_var<'stack, 'ast: 'stack>(_pos: &TextPosition, assignee: &Name, assignee_type: &Type, value: &Name, context: &mut dyn CudaContext<'stack, 'ast>) -> Result<Box<dyn CudaStatement>, OutputError> {
...
pub fn gen_array_assignment<'stack, 'ast: 'stack>(pos: &TextPosition, assignee: &Name, assignee_type: &Type, value: &Expression, context: &mut dyn CudaContext<'stack, 'ast>) -> Result<Box<dyn CudaStatement>, OutputError> {
    ...
    if assignee_type.is_view() {
        ...
        match value {
            ...
            Expression::Variable(var) => match &var.identifier {
                Identifier::BuiltIn(_) => unimplemented!(),
                Identifier::Name(val_name) if context.calculate_var_type(val_name, pos).is_view() => gen_array_checked_copy_assignment_from_var(pos, assignee, assignee_type, val_name, context),
                Identifier::Name(val_name) => {
                    let assignments = gen_array_move_assignment_from_var(pos, assignee, val_name, context).collect::<Result<Vec<_>, _>>()?;
                    Ok(Box::new(CudaBlock {
