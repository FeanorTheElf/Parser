...
use std::ops::{ Index, IndexMut, Add, Mul, AddAssign, MulAssign, SubAssign, Range, RangeFull };
use std::mem::swap;
...
impl<'a, 'b, T: 'b> Indexed<'a, usize> for MatRef<'b, T> 
{
	...
	{
		self.assert_row_in_range(index);
		self.matrix.get(index + self.rows_begin).get(self.cols_begin..self.cols_end)
	}
}
...
impl<'a, 'b, T: 'b> Indexed<'a, (Range<usize>, Range<usize>)> for MatRef<'b, T>
{
	...
	fn get(&'a self, index: (Range<usize>, Range<usize>)) -> Self::Output
	{
		self.matrix.get((self.offset_row_range(index.0), self.offset_col_range(index.1)))
	}
}
...
impl<'a, 'b, T: 'a> IndexedMut<'a, usize> for MatRefMut<'b, T> 
{
	...
	{
		self.assert_row_in_range(index);
		self.matrix.get_mut(index + self.rows.start).sub_range(self.cols.clone())
	}
}
...
impl<'a, 'b, T: 'a> IndexedMut<'a, (Range<usize>, Range<usize>)> for MatRefMut<'b, T>
{
	...
	fn get_mut(&'a mut self, index: (Range<usize>, Range<usize>)) -> Self::Output
	{
		self.matrix.get_mut((self.offset_row_range(index.0), self.offset_col_range(index.1)))
	}
}
...
fn test_matrix_transform_two_dims_left() 
{
	let mut m = Matrix::new(Box::new([1., 2., 3.,
	                                  4., 5., 6.,
								      7., 8., 9.]), 3);
	m.get_mut((.., ..)).transform_two_dims_left(0, 2, &[0., 1., 1., 0.]);
	assert_eq!(&[7., 8., 9.,
	             4., 5., 6.,
				 1., 2., 3.], m.data());
	m.get_mut((.., ..)).transform_two_dims_left(1, 2, &[0.5, 0.5, 1.0, 0.5]);
	assert_eq!(&[7.,  8.,  9.,
	             2.5, 3.5, 4.5,
				 4.5, 6.0, 7.5], m.data());
}
...
fn test_matmul() 
{
	...
	assert_eq!(&[7, 10, 7, 9, 14, 13], (a.get((.., ..)) * b.get((.., ..))).data());
}